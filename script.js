...
function calculateCapacity() {
  ...
  const bandwidthPerUser = parseFloat(
    document.getElementById("bandwidth-per-user").value
  );
  const totalBandwidth = parseFloat(
    document.getElementById("total-bandwidth").value
  );

  if (
    !validateInput(numUsers, "num-users") ||
    !validateInput(bandwidthPerUser, "bandwidth-per-user") ||
    !validateInput(totalBandwidth, "total-bandwidth")
  ) {
    return;
  }
...
function displayCapacity(requiredBandwidth, capacity) {
  ...
  capacityResultsDiv.innerHTML = `
        ...
        <p>Capacité du réseau: ${
          capacity >= 1 ? "Suffisante" : "Insuffisante"
        }</p>
